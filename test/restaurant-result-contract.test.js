...
const deny = (name) => {
  forbidden.push(name);
  throw new Error(`Isolated restaurant contract test forbids ${name}`);
};
...
const {
  createRestaurantDetailHandler,
  isStablePlaceId,
} = require("../src/routes/restaurants.routes");
...
test("exact restaurant detail retrieval uses stable place identity and the Contract v1 projection", async () => {
  const placeId = "507f1f77bcf86cd799439011";
  const place = makePlace({
    placeId,
    claims: [certificationClaim({ subject: subject(placeId), coverage: entire(placeId) })],
    evidence: [certificateEvidence({ subject: subject(placeId), coverage: entire(placeId) })],
    legacy: { halal: true, certified: true, arbitraryPrivateField: "must not escape" },
  });
  let filter;
  const PlaceModel = {
    findOne(value) {
  ...
  const handler = createRestaurantDetailHandler({ PlaceModel, logger: { error() {} } });
  const res = responseRecorder();
  await handler({ params: { id: placeId } }, res);

  assert.deepEqual(filter, { _id: placeId, type: "restaurant" });
  assert.equal(res.statusCode, 200);
  assert.equal(res.body.ok, true);
  assert.equal(res.body.item.id, placeId);
  assert.equal(res.body.item.restaurantResult.contractVersion, 1);
  assert.equal(res.body.item.restaurantResult.summary.status, "CERTIFIED");
  ...
  for (const key of ["trust", "rating", "price", "photos", "hours", "hours_raw", "distance"]) {
    assert.equal(Object.hasOwn(res.body.item, key), false, `${key} must not enter Restaurant Detail API output`);
  }
});

test("restaurant detail distinguishes not found from operational failure", async () => {
  ...
  const notFoundHandler = createRestaurantDetailHandler({
  ...
  const failedHandler = createRestaurantDetailHandler({
