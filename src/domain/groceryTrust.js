...
// Grocery Result Contract v1. Deliberately NOT a re-export of restaurantTrust.js:
// grocery coverage/claim semantics are domain-specific (a store selling some
// halal products is not an entirely halal or certified store), even though the
// normalization/eligibility architecture below mirrors the Restaurant Contract.

const GROCERY_RESULT_CONTRACT_VERSION = 1;
...
// ENTIRE_STORE, MEAT_DEPARTMENT (covers "halal meat department / butcher
// counter"), PRODUCT_CATEGORY, PRODUCT_SELECTION (an explicitly limited
// product/SKU line), NAMED_SUPPLIER, and CHAIN_LEVEL_ONLY are the only valid
// kinds; anything else normalizes to the deliberately-invalid "UNKNOWN"
// sentinel below so unestablished coverage can never pass validation.
...
const CLAIM_EVIDENCE_TYPES = Object.freeze({
  ...
  HALAL_MEAT_DEPARTMENT_CLAIM: ["BUSINESS_ATTESTATION", "SUPPLIER_DOCUMENT", "EXPERT_REVIEW"],
  PRODUCT_CATEGORY_CLAIM: ["CERTIFICATE", "BUSINESS_ATTESTATION", "PRODUCT_CATEGORY_DOCUMENT", "EXPERT_REVIEW"],
...
function normalizeCoverage(value) {
  ...
  return {
    ...
    categoryId: cleanId(coverage.categoryId),
    selectionId: cleanId(coverage.selectionId),
...
function coverageIsValid(coverage) {
  ...
  switch (coverage.kind) {
    case "ENTIRE_STORE":
    case "MEAT_DEPARTMENT":
      return Boolean(coverage.placeId);
    case "PRODUCT_CATEGORY":
      return Boolean(coverage.placeId && coverage.categoryId);
    case "PRODUCT_SELECTION":
      return Boolean(coverage.placeId && coverage.selectionId);
    case "NAMED_SUPPLIER":
...
function buildSummary(acceptedClaims, evidenceById, placeId) {
  ...
  let statement = "No accepted evidence establishes a supported halal status for this exact grocery location.";
  let supportingClaimIds = [];

  if (!hasConflict) {
    const certified = affirming.find((claim) =>
      claim.type === "INDEPENDENT_CERTIFICATION" && claim.coverage.kind === "ENTIRE_STORE" &&
      certificateForClaim(claim, evidenceById)
    );
    const confirmed = affirming.find((claim) =>
      claim.type === "BUSINESS_CONFIRMATION" && claim.coverage.kind === "ENTIRE_STORE"
    );
    const partial = affirming.filter((claim) =>
      ["INDEPENDENT_CERTIFICATION", "BUSINESS_CONFIRMATION", "HALAL_MEAT_DEPARTMENT_CLAIM", "PRODUCT_CATEGORY_CLAIM"].includes(claim.type) &&
      claim.coverage.kind !== "ENTIRE_STORE"
    );
    ...
    if (certified) {
      ...
      statement = "Independent certification evidence applies to this exact grocery location and covers the entire store.";
      supportingClaimIds = [certified.id];
    } else if (confirmed) {
      status = "BUSINESS_CONFIRMED";
      evidenceLevel = "BUSINESS_CONFIRMATION";
      statement = "An accepted owner or business confirmation applies to this exact grocery location; it is not independent certification.";
      supportingClaimIds = [confirmed.id];
    } else if (partial.length) {
      status = "PARTIAL_SUPPORT";
      evidenceLevel = "PARTIAL_EVIDENCE";
      statement = "Accepted evidence applies only to the specified coverage at this grocery location; it does not establish entire-store certification.";
      ...
      statement = "An accepted community report exists for this grocery location; it is not certification or business confirmation.";
...
function buildGroceryResultContract(value, { now = new Date() } = {}) {
  const place = toPlainObject(value);
  const raw = place.groceryTrust && typeof place.groceryTrust === "object" ? place.groceryTrust : {};
  ...
  if (summary.status === "UNVERIFIED") {
    whatWeDontKnow.push("No accepted claim establishes halal coverage for this exact grocery location.");
  }
  if (acceptedClaims.some((claim) => claim.applicability === "CHAIN_ONLY")) {
    whatWeDontKnow.push("A chain-level claim has not been established for this exact grocery location.");
  }
  if (summary.isPartial) {
    whatWeDontKnow.push("Accepted evidence does not cover the entire grocery store.");
  }
  ...
  if (exactClaims.some((claim) => claim.position === "AFFIRMS") && exactClaims.some((claim) => claim.position === "DISPUTES")) {
    limitations.push("Accepted evidence contains conflicting claims for this grocery location; no authoritative summary was selected.");
  }
  ...
  return {
    contractVersion: GROCERY_RESULT_CONTRACT_VERSION,
    identity,
    chain,
    summary,
...
function safeGroceryListing(value) {
  const place = toPlainObject(value);
  const identity = placeIdentity(place);
...
function buildGroceryListProjection(value, options) {
  const contract = buildGroceryResultContract(value, options);
  return {
    ...safeGroceryListing(value),
    grocerySummary: contract.summary,
    trust: compatibilityTrust(contract),
  };
}

function buildGroceryDetailProjection(value, options) {
  const contract = buildGroceryResultContract(value, options);
  const listing = safeGroceryListing(value);
  return {
    _id: listing._id,
    id: listing.id,
    type: listing.type,
    ...
    grocerySummary: contract.summary,
    groceryResult: contract,
  };
}

module.exports = {
  GROCERY_RESULT_CONTRACT_VERSION,
  CLAIM_TYPES,
  CLAIM_POSITIONS,
  COVERAGE_KINDS,
  EVIDENCE_TYPES,
